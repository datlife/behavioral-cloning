...
# A few comments:
# 1) PNG images were used as the input only because this was the format for round1 testset.
#    In practice, raw images should be fed directly from the rosbags.
# 2) We define get_initial_state and deep_copy_initial_state functions to be able to preserve the state of
#    our recurrent net between batches. The backpropagation is still truncated by SEQ_LEN.
# 3) The loss is composed of two components. The first is the MSE of the steering angle prediction in the autoregressive
#    setting -- that is exactly what interests us in the test time.
#    The second components, weighted by the term aux_cost_weight, is the sum of MSEs for all outputs both in
#    autoregressive and ground truth settings.

# Note: if the saver definition doesn't work for you please make sure you are using tensorflow 0.12rc0 or newer.

import tensorflow as tf
from .constants import *
from .helpers import read_csv, process_csv, apply_vision_simple, get_optimizer
